...
CREATE TABLE IF NOT EXISTS pernikahan_anggota (
  ...
  nama_pasangan     TEXT NOT NULL,       -- backup jika pasangan belum terdaftar
  urutan            INT NOT NULL DEFAULT 1,  -- ke-1, ke-2, dst
...
CREATE TABLE IF NOT EXISTS notifikasi (
  ...
  judul        TEXT NOT NULL,
  pesan        TEXT NOT NULL,
  data_id      UUID,           -- ID record yang terkait
...
-- ============================================================
-- 6. AUDIT LOG (Tidak Bisa Dihapus User Biasa)
-- ============================================================
...
-- ============================================================
-- 7. STORED FUNCTION: Statistik Dashboard
-- ============================================================
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO user_profiles (id, email, nama_lengkap, role)
  VALUES (
    NEW.id,
    NEW.email,
...
-- Audit log tidak bisa diedit/dihapus siapapun
...
-- Storage Policy: upload hanya admin
...
-- Storage Policy: semua bisa lihat foto (public)
CREATE POLICY "Publik bisa lihat foto"
  ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'foto-anggota');

-- Storage Policy: admin bisa hapus foto
...
-- Ganti 'UUID_USER_DARI_SUPABASE' dengan UUID user yang dibuat
