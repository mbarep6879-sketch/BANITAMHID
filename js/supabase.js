...
// !! WAJIB DIISI !! Ganti dengan nilai dari project Anda:
...
// PENTING: Salin SELURUH anon/public key dari Supabase Dashboard
// Settings → API → Project API keys → anon / public
// Key asli panjangnya ~200 karakter, diawali eyJhbGci...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlvZ2pkcXJ0bGtpY210d2pzb3JwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAyNTEzNDksImV4cCI6MjA5NTgyNzM0OX0._3GZzZ9Z7zWLGMYYvp4wxRjhsD8M9fBzHIIRT8_Li-w';
...
// Konversi username ke email internal Supabase
// Strategi: simpan akun Supabase Auth dengan email format
//   username@banitamhid.internal
// User hanya perlu tahu username + password, tidak perlu tahu email ini.
const toInternalEmail = (username) =>
  `${username.toLowerCase().trim()}@banitamhid.internal`;

const Auth = {

  // Login dengan username — TIDAK butuh RPC/query DB terlebih dahulu
  // Langsung konversi username → email internal → signIn
  async loginByUsername(username, password) {
    const uname = username.toLowerCase().trim();

    if (!uname) throw new Error('Username tidak boleh kosong.');
    if (password.length < 1) throw new Error('Password tidak boleh kosong.');

    // Bersihkan input — tolak karakter aneh
    ...
    if (error) {
      ...
      if (
        ...
        throw new Error('Username atau password salah. Pastikan akun sudah dibuat oleh Admin.');
      }
      throw new Error(msg || 'Login gagal. Periksa koneksi internet Anda.');
    }

    return data;
  },

  // Login dengan EMAIL langsung (untuk admin/fallback)
